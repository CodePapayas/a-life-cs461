...
class PerlinNoise2d{
	private:
		...
		int _octaves = 8;		// octaves of detail; how granular is the noise
		double _frequency = 1;	// initial frequency- or scale- of the noise; how "zoomed in" is it
		double _amplitude = 1;	// initial amplitude; how contrasted is the noise, smoothed or sharp?
	...
	public:		
		...
		double SampleLayered(Vector2d pos){
			// Input: Vector2d pos
			// Output: double result
			...
			double curr_freq 	= _frequency;
			double curr_amp 	= _amplitude;
			for(int o = 1; o < _octaves; o++){
				Vector2d freq_pos = Vector2d(curr_freq * pos.x, curr_freq * pos.y);
				curr_sample += curr_amp * Sample(freq_pos);
				curr_amp *= 0.5;
				curr_freq *= 2.0;
			}
			return curr_sample;
		...
		double SampleNormalized(Vector2d pos){
			double noise_val = SampleLayered(pos);
            noise_val += 2 * _amplitude;
            noise_val /= 4 * _amplitude;
			return noise_val;
		}
		...
		void regeneratePermutation() {makePermutation();};
		...
		PerlinNoise2d(int s, double f, double a, int o){
			_seed = s; 
			_frequency = f;
			_amplitude = a;
			_octaves = o;
		...
		PerlinNoise2d(double f, double a, int o){
			_frequency = f;
			_amplitude = a;
			_octaves = o;
			makePermutation();
